...
import { createBuilder, createLogger, createServer, isCSSRequest, mergeConfig, transformWithOxc } from "vite";
import { basename, dirname, isAbsolute, join, normalize, relative, resolve } from "pathe";
import { bundlerDiagnostics, createIsIgnored, directoryToURL, getLayerDirectories, logger, recoverThrottledChanges, resolveAlias, resolvePath, setBuildOutput, tryUseNuxt, useNitro, useNuxt } from "@nuxt/kit";
...
//#endregion
//#region ../nuxt/src/core/plugins/virtual.ts
const PREFIX$1 = "virtual:nuxt:";
function toVirtualId(absolutePath, nuxt) {
	return PREFIX$1 + encodeURIComponent(relative(nuxt.options.rootDir, absolutePath));
}
//#endregion
...
*/
function resolveClientManifestFile(manifest) {
	if (!manifest) throw bundlerDiagnostics.NUXT_B7020();
...
* that imports them.
*/
function resolveGlobalCss(nuxt) {
	...
	for (const entry of nuxt.options.css) {
		...
		if (fromModules) out.set(fromModules, toFsUrl(fromModules));
	}
	return Array.from(out, ([file, url]) => ({
		file,
		url
	}));
}
...
*/
function collectDevCss(nuxt, moduleGraph) {
	...
	for (const entry of resolveGlobalCss(nuxt)) if (!files.has(entry.file)) urls.add(entry.url);
	return [...urls];
}
//#endregion
//#region src/plugins/vite-node.ts
function getManifest(nuxt, viteServer, clientEntry) {
	const css = collectDevCss(nuxt, (nuxt.options.experimental.viteEnvironmentApi ? viteServer.environments.ssr : viteServer).moduleGraph);
...
function ViteNodePlugin(nuxt) {
	...
	const rollupConfig = nitro.options.rollupConfig ||= {};
	const existingExternal = rollupConfig.external;
	rollupConfig.external = (id, ...args) => {
		if (externalRuntimeUrls.has(id)) return true;
		if (typeof existingExternal === "function") return existingExternal(id, ...args);
		return (existingExternal == null ? [] : Array.isArray(existingExternal) ? existingExternal : [existingExternal]).some((e) => typeof e === "string" ? e === id : e.test(id));
	};
	...
	return {
		...
		handleHotUpdate({ server: clientServer, timestamp }) {
			...
			for (const mod of clientServer.moduleGraph.idToModuleMap.values()) {
				...
				if (ssrMod) ssrGraph.invalidateModule(ssrMod, void 0, timestamp);
			}
		},
		async configureServer(clientServer) {
			...
			function collectInvalidatedSsrModules(ssrServer) {
				...
				for (const [file, remaining] of pendingChangedFiles) {
					...
					if (!mods?.size) {
						if (remaining <= 1) pendingChangedFiles.delete(file);
						else pendingChangedFiles.set(file, remaining - 1);
						continue;
					}
					let invalidatedVirtual = false;
					const invalidateModule = (mod) => ssrModuleGraph.invalidateModule(mod);
					const seen = /* @__PURE__ */ new Set();
					const invalidateVirtualImports = (mod) => {
					...
					if (invalidatedVirtual || remaining <= 1) pendingChangedFiles.delete(file);
					else pendingChangedFiles.set(file, remaining - 1);
				}
			}
			...
			nuxt.hook("app:templatesGenerated", (_app, changedTemplates) => {
				const ssrModuleGraph = nuxt.options.experimental.viteEnvironmentApi ? clientServer.environments.ssr.moduleGraph : legacySsrServer?.moduleGraph;
...
function createViteNodeSocketServer(nuxt, ssrServer, clientServer, invalidates, collectInvalidatedSsrModules, config) {
	const server = net.createServer((socket) => {
				...
				switch (request.type) {
					...
					case "module": {
						...
						if (response && !response.map) {
							...
							const mod = graph.getModuleById(request.payload.moduleId) || graph.fileToModulesMap.get(request.payload.moduleId)?.values().next().value;
							if (mod?.transformResult?.map) response.map = mod.transformResult.map;
						}
		...
		const ensureBufferCapacity = (additionalBytes) => {
			...
			if (requiredSize > MAX_BUFFER_SIZE) throw bundlerDiagnostics.NUXT_B7012({
				requiredSize,
	...
	if (!currentSocketPath) throw bundlerDiagnostics.NUXT_B7013();
...
const PublicDirsPlugin = (options) => {
	...
	return [{
		...
		renderChunk(code, chunk) {
			...
			if (s.hasChanged()) s.prepend(`import { publicAssetsURL } from '#internal/nuxt/paths';`);
...
//#endregion
//#region src/plugins/optimize-deps-hint.ts
...
function OptimizeDepsHintPlugin(nuxt) {
	...
	const getSnippetDeps = () => [.../* @__PURE__ */ new Set([...getUserInclude().filter((d) => !userStale.has(d)), ...discovered])];
	...
	function scheduleHint() {
		...
		hintTimer = setTimeout(() => {
			...
			if (hasNew) {
				...
				hasShownStaleHint = true;
				const deps = [...[...userStale].map((d) => `\`${d}\``), ...[...moduleStale].map((d) => `\`${d}\` (from a Nuxt module)`)].join(", ");
...
function StableEntryPlugin(nuxt) {
	...
	return {
		...
		applyToEnvironment(environment) {
			...
			return toArray(environment.config.build.rolldownOptions?.output).some((output) => typeof output?.entryFileNames === "string" && output?.entryFileNames.includes("[hash]"));
		},
			...
			return [{
				...
				async generateBundle(_opts, outputBundle) {
					for (const _bundleId in outputBundle) {
						...
						for (const [moduleId, module] of Object.entries(bundle.modules)) minifiedModuleEntryPromises.push(transformWithOxc(module.code || "", _bundleId, {}, void 0, config).then((result) => [moduleId, {
							...module,
							code: result.code
						}]));
...
const resolvedClientBuild = /* @__PURE__ */ new WeakMap();
/**
...
* has started.
*/
function getResolvedClientBuild(nuxt) {
...
async function writeManifest(ctx) {
	...
	const clientBuild = getResolvedClientBuild(nuxt) ?? {
		outDir: clientDist,
		manifest: "manifest.json"
	};
	const manifestFile = nuxt.options.dev ? "" : resolve(clientBuild.outDir, resolveClientManifestFile(clientBuild.manifest));
	const clientManifest = nuxt.options.dev ? devClientManifest : JSON.parse(readManifestFromDisk(manifestFile));
...
function readManifestFromDisk(manifestFile) {
	try {
		return readFileSync(manifestFile, "utf-8");
	} catch (error) {
		if (error.code === "ENOENT") throw bundlerDiagnostics.NUXT_B7021({ manifestFile });
		throw error;
	}
}
//#endregion
...
function detachSsrHmrServer(server, hmrPort) {
	...
	const userPort = ws.port || (typeof server.hmr === "object" ? server.hmr?.port : void 0);
	server.hmr = false;
	server.ws = {
		...ws,
		server: void 0,
		port: userPort || hmrPort
	};
}
			...
			return {
				...
				renderChunk(_code, chunk) {
					...
					for (const moduleId of [chunk.facadeModuleId, ...chunk.moduleIds].filter(Boolean)) {
						...
						if (relativePath in cssMap) cssMap[relativePath].inBundle = cssMap[relativePath].inBundle ?? (isVue(stripQuery(moduleId)) && !!relativePath || isEntry);
					}
					return null;
				},
				transform: {
...
//#endregion
//#region src/plugins/decorators.ts
...
function hasDecoratorSyntax(code, jsx = false) {
	...
	for (const token of tokens) {
		...
		switch (token.type) {
			...
			case "TemplateHead":
			case "TemplateMiddle": continue;
			case "MultiLineComment":
			case "StringLiteral":
			case "NoSubstitutionTemplate":
			case "TemplateTail":
			case "JSXString": if (token.closed) continue;
		}
		return true;
	}
	return false;
	...
	return {
		...
		transform: {
			...
			handler(code, id) {
				...
				if (!hasDecoratorSyntax(code, /\.[jt]sx(?:$|\?)/.test(id))) return;
...
function EnvironmentsPlugin(nuxt) {
	...
	return {
		...
		configEnvironment(name, config) {
			...
			if (name === "client") {
				const outputConfig = config.build?.rolldownOptions?.output;
...
//#endregion
//#region src/plugins/server-entry.ts
function ServerEntryPlugin(nuxt) {
	if (nuxt.options.dev) return;
...
function ClientManifestPlugin(nuxt) {
	...
	return {
		...
		async closeBundle() {
			...
			const clientManifest = nuxt.options.dev ? devClientManifest : JSON.parse(readManifestFromDisk());
			const manifestEntries = Object.values(clientManifest);
			const buildAssetsDir = withTrailingSlash(withoutLeadingSlash(nuxt.options.app.buildAssetsDir));
			const BASE_RE = new RegExp(`^${escapeStringRegexp(buildAssetsDir)}`);
			for (const entry of manifestEntries) {
...
function ResolveDeepImportsPlugin(nuxt) {
	const EXCLUDE_RE = new RegExp("^(?:" + [
		...
		"/__skip_vite",
		"@vitest/"
	].map((e) => escapeStringRegexp(e)).join("|") + ")");
	const conditions = {};
	...
	return {
		...
		enforce: "post",
		resolveId: {
			filter: { id: { exclude: [ABSOLUTE_RE, EXCLUDE_RE] } },
			async handler(id, importer, options) {
				if (options.isEntry && HTML_RE.test(id)) return;
				if (!importer || !ABSOLUTE_OR_VIRTUAL_RE.test(importer)) return;
...
const bundle = async (nuxt) => {
	...
	const assetFileNames = nuxt.options.dev ? void 0 : (chunk) => withoutLeadingSlash(join(nuxt.options.app.buildAssetsDir, `${sanitizeFilePath(filename(chunk.names[0]))}.[hash].[ext]`));
...
const setupWatcher = (nuxt) => {
	...
	nuxt.hook("vite:serverCreated", (server, { isClient }) => {
		...
		if (extraPaths.size) {
			...
			for (const delay of RECONCILE_DELAYS) setTimeout(() => {
