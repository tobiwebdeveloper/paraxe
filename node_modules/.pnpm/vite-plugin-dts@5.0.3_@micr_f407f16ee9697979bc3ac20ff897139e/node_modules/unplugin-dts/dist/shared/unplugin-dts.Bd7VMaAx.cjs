...
function tryGetPackageInfo(name) {
  if (process.versions.pnp) {
    const targetRequire = require$$0.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('shared/unplugin-dts.Bd7VMaAx.cjs', document.baseURI).href)));
    try {
...
function createApiExtractorProvider({
  configPath,
  extractorConfig,
  bundledPackages,
  invokeOptions
...
class Runtime {
  ...
  constructor(options, { createParsedCommandLine, createProgram }) {
    ...
    const emittableSourceFiles = program.getSourceFiles().filter(maybeEmitted).map((sourceFile) => sourceFile.fileName);
    const sourcePublicPath = queryPublicPath(emittableSourceFiles);
    const useTs6ImplicitRootDir = !compilerOptions.rootDir && !(compilerOptions.composite && compilerOptions.configFilePath) && compareVersions.compare(ts.version, "6.0.0", ">=") && !!configPath;
    let publicRoot = compilerOptions.rootDir ? ensureAbsolute(resolveConfigDir(compilerOptions.rootDir, root), root) : compilerOptions.composite && compilerOptions.configFilePath ? node_path.dirname(compilerOptions.configFilePath) : useTs6ImplicitRootDir ? node_path.dirname(configPath) : sourcePublicPath || (configPath ? node_path.dirname(configPath) : root);
...
`
          ...
          const bundleEntry = async (path) => {
            ...
            if (typeof afterRollup === "function") {
              await unwrapPromise(afterRollup(result.meta));
            }
          };
          if (multiple) {
